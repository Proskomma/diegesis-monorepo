...
import { directionText, otherDirectionText, setFontFamily } from "../i18n/languageDirection";
          ...
          <Grid item xs={12} sm={6} md={4} lg={3} key={uploadedFile.name}>
            <Card style={{ marginTop: "5%" }}>
              <CardContent dir={directionText(appLang)}>
                <Typography sx={{ mb: 1.5 }} color="text.secondary" style={{ fontFamily : setFontFamily(appLang)}}>
                  {uploadedFile.name}
                    {" ("}
                    {`${uploadedFile.content.length} ${i18n(appLang, "BYTES")}`}
                    {")"}
                </Typography>
                <Typography sx={{ mb: 1.5 }} color="text.secondary"  dir={directionText(appLang)} style={{ fontFamily : setFontFamily(appLang)}}>

...
const readFlexibleUIConfig = (config, id, langCode) => {
    try {
        const compUIConfigPath = path.join(uiConfigDir(config.uiConfigPath, translationDir(langCode)), `${id}.json`);
...
const writeFlexibleUIConfig = (config, objData) => {
    try {
        const originPath = uiConfigDir(config.uiConfigPath, objData.langCode);
        if (!fse.pathExistsSync(originPath)) {
            fse.mkdirsSync(originPath);
        }
